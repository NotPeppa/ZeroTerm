...
/// Host identity presented by the server during key exchange.
///
/// Keeping certificates distinct from ordinary public keys lets clients
/// implement OpenSSH `@cert-authority` trust without weakening the default
/// behavior: handlers which do not opt in still reject certificates.
#[derive(Debug, Clone)]
pub enum ServerHostKey {
    PublicKey(PublicKey),
                        ...
                        if let Some(server_host_key) = &server_host_key {
                            let check = match server_host_key {
                                ...
                                ServerHostKey::Certificate(certificate) => {
                                    handler.check_server_certificate(certificate).await?
                                }
                            };
                            if !check {
...
mod tests {
    ...
    use crate::{cipher, mac, CryptoVec};
...
pub trait Handler: Sized + Send {
    ...
    /// Called to check an OpenSSH host certificate. The default
    /// implementation rejects all certificates.
    #[allow(unused_variables)]
    fn check_server_certificate(
        &mut self,
        server_certificate: &ssh_key::Certificate,
    ) -> impl Future<Output = Result<bool, Self::Error>> + Send {
        async { Ok(false) }
    }
