...
use crate::client::{Config, NewKeys, ServerHostKey};
    ...
    pub fn step(
        ...
        match self.state {
            ...
            ClientKexState::WaitingForDhReply { mut names, mut kex } => {
                ...
                let server_host_key = if names.key.as_str().ends_with("-cert-v01@openssh.com") {
                    let certificate = Certificate::from_bytes(&server_host_key_bytes)?;
                    debug!(
                        "received server host certificate: {:?}",
                        certificate.to_openssh()
                    );
                    ServerHostKey::Certificate(certificate)
                } else {
                    let public_key = parse_public_key(&server_host_key_bytes)?;
                    debug!("received server host key: {:?}", public_key.to_openssh());
                    ServerHostKey::PublicKey(public_key)
                };
                ...
                match &server_host_key {
                    ServerHostKey::PublicKey(key) => key.to_bytes()?.encode(&mut pubkey_vec)?,
                    ServerHostKey::Certificate(certificate) => {
                        certificate.to_bytes()?.encode(&mut pubkey_vec)?
                    }
                }
                ...
                let verification = match &server_host_key {
                    ServerHostKey::PublicKey(key) => {
                        signature::Verifier::verify(key, hash.as_ref(), &signature)
                    }
                    ServerHostKey::Certificate(certificate) => signature::Verifier::verify(
